...
// It identifies column headers and aligns data accordingly, returning it in JSON format.
...
// - ExtractPdfDataOutput - The output type, a JSON string containing the extracted data.
...
const ExtractPdfDataInputSchema = z.object({
  pdfDataUri: z
    .string()
    .describe(
      'The PDF document as a data URI, including MIME type and Base64 encoding (e.g., data:application/pdf;base64,<encoded_data>)'
    ),
});
...
const extractPdfDataPrompt = ai.definePrompt({
  ...
  Your task is to extract structured data from the provided PDF document.
  Identify column headers and align the corresponding data accordingly.
  Return the extracted data as a JSON string.

  Here is the PDF document:
  {{media url=pdfDataUri}}

  Ensure the JSON format is valid and accurately represents the data within the PDF.
  `, config: {
    safetySettings: [
      {
